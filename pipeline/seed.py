...
def build_frames(con: duckdb.DuckDBPyConnection) -> None:
    ...
    #
    # updated_at is the purchase instant of that earliest order, not now(). It
    # is the business time the address became true, which is what SCD2 windows
    # downstream need; wall clock would compress all history into one instant.
    con.execute(
        """
        ...
        WITH ranked AS (
            SELECT c.customer_unique_id,
                   ...
                   o.order_purchase_timestamp AS updated_at,
                   ROW_NUMBER() OVER (
        ...
        SELECT customer_unique_id, zip_code_prefix, city, state, updated_at
        FROM ranked WHERE rn = 1
        """
    )

    # Sellers have no event of their own in the source, so they are anchored to
    # the earliest order they fulfilled.
    con.execute(
        """
        ...
        SELECT s.seller_id,
               ...
               COALESCE(MIN(o.order_purchase_timestamp),
                        TIMESTAMP '2016-09-01 00:00:00') AS updated_at
        FROM raw_sellers s
        LEFT JOIN raw_items i ON i.seller_id = s.seller_id
        LEFT JOIN raw_orders o ON o.order_id = i.order_id
        GROUP BY 1, 2, 3, 4
        """
    )
...
LOAD_ORDER = [
    ("customer", "t_customer",
     ["customer_unique_id", "zip_code_prefix", "city", "state", "updated_at"]),
    ("seller", "t_seller",
     ["seller_id", "zip_code_prefix", "city", "state", "updated_at"]),
    ("product", "t_product",
     ["product_id", "category_name", "category_name_en",
      "weight_g", "length_cm", "height_cm", "width_cm", "updated_at"]),
    ("order", "t_order",
     ["order_id", "customer_id", "customer_unique_id", "order_status",
      "purchased_at", "approved_at", "delivered_carrier_at",
      "delivered_customer_at", "estimated_delivery_at", "updated_at"]),
    ("order_item", "t_order_item",
     ["order_id", "order_item_id", "product_id", "seller_id",
      "shipping_limit_at", "price", "freight_value", "updated_at"]),
]
